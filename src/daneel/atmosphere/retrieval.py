        ...
        with open(self.output_params, 'w') as f:
            ...
            for sol_idx, map_vals, median, _ in opt.get_solution():
                ...
                if plot:
                    ...
                    ax.errorbar(obs.wavelengthGrid, obs.spectrum, yerr=obs.errorBar, label = 'Observations', fmt = '.', color = 'black', alpha = 0.6, zorder = 1)
                    sort_args = np.argsort(wl_microns)
                    ax.plot(wl_microns[sort_args], bin_depth[sort_args], label = 'Best Fit (Retrieved)', color = 'red', linewidth = 2, zorder = 2)
                    ax.set_xscale('log')
